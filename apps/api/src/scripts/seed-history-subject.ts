...
import { HistorySubjectSeeder } from '../modules/curriculum/infrastructure/history-subject.seeder.js';
...
// `pnpm --filter @aletheia/api run seed:history-subject` to (re)publish
// the "História" domain, its grade-band paths and competencies. Safe to
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `History subject seed: domain ${result.domainCreated ? 'created' : 'already present'}, ` +
