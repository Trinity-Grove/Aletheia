...
import { GeographySubjectSeeder } from '../modules/curriculum/infrastructure/geography-subject.seeder.js';
...
// `pnpm --filter @aletheia/api run seed:geography-subject` to (re)publish
// the "Geografia" domain, its grade-band paths and competencies. Safe to
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Geography subject seed: domain ${result.domainCreated ? 'created' : 'already present'}, ` +
