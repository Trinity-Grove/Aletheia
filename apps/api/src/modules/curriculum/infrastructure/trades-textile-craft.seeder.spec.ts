...
import { TradesTextileCraftSeeder } from './trades-textile-craft.seeder.js';
import { buildTradesTextileCraftSeedData } from './trades-textile-craft.seed-data.js';

const DOMAIN_ID = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a14';

const seedDataForPathIds = buildTradesTextileCraftSeedData();
const PATH_ID_BY_CODE = new Map(
  seedDataForPathIds.paths.map((p, i) => [p.path.code, `b0eebc99-9c0b-4ef8-bb6d-6bb9bd380d${String(i).padStart(2, '0')}`]),
);
...
describe('TradesTextileCraftSeeder', () => {
  ...
  it('creates every trade path and its competencies when the TRADES domain already exists (the normal case)', async () => {
    ...
    const seeder = new TradesTextileCraftSeeder(definitionsService);
