...
import { CookingProgressionSeeder } from './cooking-progression.seeder.js';
import { buildCookingProgressionSeedData } from './cooking-progression.seed-data.js';

const DOMAIN_ID = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380b11';
const PATH_ID = 'b0eebc99-9c0b-4ef8-bb6d-6bb9bd380b22';

describe('CookingProgressionSeeder', () => {
  const seedData = buildCookingProgressionSeedData();

  it('creates the path and every competency when the COOKING domain already exists (the normal case)', async () => {
    ...
    const seeder = new CookingProgressionSeeder(definitionsService);
  ...
  it('creates the COOKING domain defensively if it does not exist yet (standalone run on a fresh database)', async () => {
    const definitionsService = {
