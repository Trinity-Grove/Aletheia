...
import { ResilienceNavigationCampingSeeder } from './resilience-navigation-camping.seeder.js';
import { buildResilienceNavigationCampingSeedData } from './resilience-navigation-camping.seed-data.js';

const DOMAIN_ID = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a15';

const seedDataForPathIds = buildResilienceNavigationCampingSeedData();
const PATH_ID_BY_CODE = new Map(
  seedDataForPathIds.paths.map((p, i) => [p.path.code, `b0eebc99-9c0b-4ef8-bb6d-6bb9bd380e${String(i).padStart(2, '0')}`]),
);
...
describe('ResilienceNavigationCampingSeeder', () => {
  ...
  it('creates every path and its competencies when the RESILIENCE domain already exists (the normal case)', async () => {
    ...
    const seeder = new ResilienceNavigationCampingSeeder(definitionsService);
