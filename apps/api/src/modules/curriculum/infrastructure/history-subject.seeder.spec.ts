...
import { HistorySubjectSeeder } from './history-subject.seeder.js';
import { buildHistorySubjectSeedData } from './history-subject.seed-data.js';
...
// history-subject.seed-data.ts eventually adds, with no collision risk.
const seedDataForPathIds = buildHistorySubjectSeedData();
...
describe('HistorySubjectSeeder', () => {
  ...
  it('creates the domain, every grade-band path and every competency on a first run', async () => {
    ...
    const seeder = new HistorySubjectSeeder(definitionsService);
