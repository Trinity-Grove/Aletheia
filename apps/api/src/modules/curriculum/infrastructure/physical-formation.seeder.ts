...
import {
  buildPhysicalFormationSeedData,
  buildPhysicalFormationDomainDto,
  buildPhysicalFormationPathDto,
  buildPhysicalFormationCompetencyDto,
} from './physical-formation.seed-data.js';

export interface PhysicalFormationSeedResult {
  domainCreated: boolean;
  pathCreated: boolean;
  competenciesCreated: number;
}

// Installs the "Formação Física" foundational content from issue #95
// section 23 (domain -> path -> competencies) -- same pattern as
// GardeningFormationSeeder (issue #95 section 19, PR #136) and the rest
// of the Domain -> Path -> Competency seeders.
//
...
@Injectable()
export class PhysicalFormationSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<PhysicalFormationSeedResult> {
    const data = buildPhysicalFormationSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(buildPhysicalFormationDomainDto(data.domain));
    ...
    if (!path) {
      path = await this.definitionsService.createLearningPath(buildPhysicalFormationPathDto(data.path, domain.id));
    ...
    for (const competencySeed of data.competencies) {
      ...
      const created = await this.definitionsService.createCompetencyDefinition(
        buildPhysicalFormationCompetencyDto(competencySeed, domain.id, path.id),
      );
