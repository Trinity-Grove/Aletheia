...
import {
  buildGardeningFormationSeedData,
  buildGardeningFormationDomainDto,
  buildGardeningFormationPathDto,
  buildGardeningFormationCompetencyDto,
} from './gardening-formation.seed-data.js';

export interface GardeningFormationSeedResult {
  domainCreated: boolean;
  pathCreated: boolean;
  competenciesCreated: number;
}

// Installs the "Plantio" foundational content from issue #95 section 19
// (domain -> path -> competencies) -- same pattern as
// BiblicalFormationSeeder (issue #95 section 5, PR #131),
// MusicFormationSeeder (issue #95 section 13, PR #132),
// TradesFormationSeeder (issue #95 section 15, PR #134), and
// CookingFormationSeeder (issue #95 section 18, PR #135).
//
...
@Injectable()
export class GardeningFormationSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<GardeningFormationSeedResult> {
    const data = buildGardeningFormationSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(buildGardeningFormationDomainDto(data.domain));
    ...
    if (!path) {
      path = await this.definitionsService.createLearningPath(buildGardeningFormationPathDto(data.path, domain.id));
    ...
    for (const competencySeed of data.competencies) {
      ...
      const created = await this.definitionsService.createCompetencyDefinition(
        buildGardeningFormationCompetencyDto(competencySeed, domain.id, path.id),
      );
