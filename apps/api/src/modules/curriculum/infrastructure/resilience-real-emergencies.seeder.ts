...
import {
  buildResilienceRealEmergenciesSeedData,
  buildResilienceRealEmergenciesDomainDto,
  buildResilienceRealEmergenciesPathDto,
  buildResilienceRealEmergenciesCompetencyDto,
} from './resilience-real-emergencies.seed-data.js';

export interface ResilienceRealEmergenciesSeedResult {
  domainCreated: boolean;
  pathsCreated: number;
  competenciesCreated: number;
}

// Installs the "Emergências Reais" LearningPath under the existing
// `RESILIENCE` LearningDomain (PR #137). Last of 3 sibling seeders
// covering issue #95 section 20's remaining five subsections -- see
// resilience-real-emergencies.seed-data.ts for the full rationale.
//
...
@Injectable()
export class ResilienceRealEmergenciesSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<ResilienceRealEmergenciesSeedResult> {
    const data = buildResilienceRealEmergenciesSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(
        buildResilienceRealEmergenciesDomainDto(data.domain),
      );
    ...
    for (const pathData of data.paths) {
      ...
      if (!path) {
        path = await this.definitionsService.createLearningPath(
          buildResilienceRealEmergenciesPathDto(pathData.path, domain.id),
        );
      ...
      for (const competencySeed of pathData.competencies) {
        ...
        const created = await this.definitionsService.createCompetencyDefinition(
          buildResilienceRealEmergenciesCompetencyDto(competencySeed, domain.id, path.id),
        );
