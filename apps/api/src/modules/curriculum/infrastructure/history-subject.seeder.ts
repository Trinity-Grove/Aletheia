...
import {
  buildHistorySubjectSeedData,
  buildHistorySubjectDomainDto,
  buildHistorySubjectPathDto,
  buildHistorySubjectCompetencyDto,
} from './history-subject.seed-data.js';

export interface HistorySubjectSeedResult {
  domainCreated: boolean;
  pathsCreated: number;
  competenciesCreated: number;
}

// Installs the "História" core-academic-subject content (domain ->
// multiple grade-band paths -> competencies). Third core subject, after
// Matemática (#142/#145) and Ciências (#147) -- see
// history-subject.seed-data.ts for the full rationale. Same shape as
// MathSubjectSeeder/ScienceSubjectSeeder: this domain has multiple
// LearningPaths (one per grade band), so the seeding loop is one level
// deeper than the single-path enrichment-domain seeders.
//
...
// window.
@Injectable()
export class HistorySubjectSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<HistorySubjectSeedResult> {
    const data = buildHistorySubjectSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(buildHistorySubjectDomainDto(data.domain));
    ...
    for (const pathData of data.paths) {
      ...
      if (!path) {
        path = await this.definitionsService.createLearningPath(
          buildHistorySubjectPathDto(pathData.path, domain.id),
        );
      ...
      for (const competencySeed of pathData.competencies) {
        ...
        const created = await this.definitionsService.createCompetencyDefinition(
          buildHistorySubjectCompetencyDto(competencySeed, domain.id, path.id),
        );
