...
import {
  buildScienceSubjectSeedData,
  buildScienceSubjectDomainDto,
  buildScienceSubjectPathDto,
  buildScienceSubjectCompetencyDto,
} from './science-subject.seed-data.js';

export interface ScienceSubjectSeedResult {
  domainCreated: boolean;
  pathsCreated: number;
  competenciesCreated: number;
}

// Installs the "Ciências" core-academic-subject content (domain ->
// multiple grade-band paths -> competencies). Second core subject after
// Matemática (#142/#145) -- see science-subject.seed-data.ts for the
// full rationale. Same shape as MathSubjectSeeder: this domain has
// multiple LearningPaths (one per grade band), so the seeding loop is
// one level deeper than the single-path enrichment-domain seeders.
//
...
// window.
@Injectable()
export class ScienceSubjectSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<ScienceSubjectSeedResult> {
    const data = buildScienceSubjectSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(buildScienceSubjectDomainDto(data.domain));
    ...
    for (const pathData of data.paths) {
      ...
      if (!path) {
        path = await this.definitionsService.createLearningPath(
          buildScienceSubjectPathDto(pathData.path, domain.id),
        );
      ...
      for (const competencySeed of pathData.competencies) {
        ...
        const created = await this.definitionsService.createCompetencyDefinition(
          buildScienceSubjectCompetencyDto(competencySeed, domain.id, path.id),
        );
