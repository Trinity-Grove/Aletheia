...
import {
  buildBiblicalFormationIntermediateSeedData,
  buildBiblicalFormationIntermediateDomainDto,
  buildBiblicalFormationIntermediatePathDto,
  buildBiblicalFormationIntermediateCompetencyDto,
} from './biblical-formation-intermediate.seed-data.js';

export interface BiblicalFormationIntermediateSeedResult {
  domainCreated: boolean;
  pathCreated: boolean;
  competenciesCreated: number;
}

// Installs the "Progressão" (intermediate tier) LearningPath
// (FAITH.BIBLICAL_FORMATION.INTERMEDIATE) under the existing
// FAITH.BIBLICAL_FORMATION LearningDomain (PR #131). Same pattern as
// CookingProgressionSeeder relative to CookingFormationSeeder.
//
// Domain handling is defensive/idempotent: if FAITH.BIBLICAL_FORMATION
// already exists (the normal case, since BiblicalFormationSeeder runs
// first in every real deployment), it's found and left untouched; if
...
// order never matters.
@Injectable()
export class BiblicalFormationIntermediateSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<BiblicalFormationIntermediateSeedResult> {
    const data = buildBiblicalFormationIntermediateSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(
        buildBiblicalFormationIntermediateDomainDto(data.domain),
      );
    ...
    if (!path) {
      path = await this.definitionsService.createLearningPath(
        buildBiblicalFormationIntermediatePathDto(data.path, domain.id),
      );
    ...
    for (const competencySeed of data.competencies) {
      ...
      const created = await this.definitionsService.createCompetencyDefinition(
        buildBiblicalFormationIntermediateCompetencyDto(competencySeed, domain.id, path.id),
      );
