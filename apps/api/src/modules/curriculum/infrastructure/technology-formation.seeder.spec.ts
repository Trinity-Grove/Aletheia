...
import { TechnologyFormationSeeder } from './technology-formation.seeder.js';
import { buildTechnologyFormationSeedData } from './technology-formation.seed-data.js';

const DOMAIN_ID = 'c0eebc99-9c0b-4ef8-bb6d-6bb9bd380a16';

const seedDataForPathIds = buildTechnologyFormationSeedData();
const PATH_ID_BY_CODE = new Map(
  seedDataForPathIds.paths.map((p, i) => [p.path.code, `d0eebc99-9c0b-4ef8-bb6d-6bb9bd380f${String(i).padStart(2, '0')}`]),
);
...
describe('TechnologyFormationSeeder', () => {
  ...
  it('creates the domain, both paths and every competency on a first run', async () => {
    ...
    const seeder = new TechnologyFormationSeeder(definitionsService);
