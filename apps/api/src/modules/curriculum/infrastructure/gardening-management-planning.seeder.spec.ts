...
import { GardeningManagementPlanningSeeder } from './gardening-management-planning.seeder.js';
import { buildGardeningManagementPlanningSeedData } from './gardening-management-planning.seed-data.js';

const DOMAIN_ID = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380c11';

const seedDataForPathIds = buildGardeningManagementPlanningSeedData();
...
describe('GardeningManagementPlanningSeeder', () => {
  ...
  it('creates every path and its competencies when the GARDENING domain already exists (the normal case)', async () => {
    ...
    const seeder = new GardeningManagementPlanningSeeder(definitionsService);
  ...
  it('creates the GARDENING domain defensively if it does not exist yet (standalone run on a fresh database)', async () => {
    const definitionsService = {
