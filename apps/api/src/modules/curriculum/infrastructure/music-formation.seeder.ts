...
import {
  buildMusicFormationSeedData,
  buildMusicFormationDomainDto,
  buildMusicFormationPathDto,
  buildMusicFormationCompetencyDto,
} from './music-formation.seed-data.js';

export interface MusicFormationSeedResult {
  domainCreated: boolean;
  pathCreated: boolean;
  competenciesCreated: number;
}

// Installs the "Musicalização" foundational content from issue #95
// section 13 (domain -> path -> competencies) -- same pattern as
...
@Injectable()
export class MusicFormationSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<MusicFormationSeedResult> {
    const data = buildMusicFormationSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(buildMusicFormationDomainDto(data.domain));
    ...
    if (!path) {
      path = await this.definitionsService.createLearningPath(buildMusicFormationPathDto(data.path, domain.id));
    ...
    for (const competencySeed of data.competencies) {
      ...
      const created = await this.definitionsService.createCompetencyDefinition(
        buildMusicFormationCompetencyDto(competencySeed, domain.id, path.id),
      );
