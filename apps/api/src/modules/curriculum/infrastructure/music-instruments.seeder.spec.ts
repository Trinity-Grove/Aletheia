...
import { MusicInstrumentsSeeder } from './music-instruments.seeder.js';
import { buildMusicInstrumentsSeedData } from './music-instruments.seed-data.js';

const DOMAIN_ID = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a15';

const seedDataForPathIds = buildMusicInstrumentsSeedData();
...
describe('MusicInstrumentsSeeder', () => {
  ...
  it('creates every instrument path and its competencies when the MUSIC domain already exists (the normal case)', async () => {
    ...
    const seeder = new MusicInstrumentsSeeder(definitionsService);
  ...
  it('creates the MUSIC domain defensively if it does not exist yet (standalone run on a fresh database)', async () => {
    const definitionsService = {
  ...
  it('creates only the missing paths and their competencies when some instrument paths already exist', async () => {
    const existingDomain = { id: DOMAIN_ID, code: seedData.domain.code, version: 1, status: 'PUBLISHED' };
