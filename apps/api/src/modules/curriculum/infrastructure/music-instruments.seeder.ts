...
import {
  buildMusicInstrumentsSeedData,
  buildMusicInstrumentsDomainDto,
  buildMusicInstrumentsPathDto,
  buildMusicInstrumentsCompetencyDto,
} from './music-instruments.seed-data.js';

export interface MusicInstrumentsSeedResult {
  domainCreated: boolean;
  pathsCreated: number;
  competenciesCreated: number;
}

// Installs the nine per-instrument "Instrumentos" LearningPaths (issue
// #95 section 13) under the existing `MUSIC` LearningDomain (PR #132).
// Same defensive/idempotent pattern as TradesTextileCraftSeeder (PR
// #153): if `MUSIC` already exists, it's found and left untouched; if
// this seeder happens to run first on a fresh database, it creates the
// domain using the exact same seed MusicFormationSeeder uses, so run
// order never matters. See music-instruments.seed-data.ts for the full
// rationale behind the nine instrument paths chosen.
@Injectable()
export class MusicInstrumentsSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<MusicInstrumentsSeedResult> {
    const data = buildMusicInstrumentsSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(buildMusicInstrumentsDomainDto(data.domain));
    ...
    for (const pathData of data.paths) {
      ...
      if (!path) {
        path = await this.definitionsService.createLearningPath(
          buildMusicInstrumentsPathDto(pathData.path, domain.id),
        );
      ...
      for (const competencySeed of pathData.competencies) {
        ...
        const created = await this.definitionsService.createCompetencyDefinition(
          buildMusicInstrumentsCompetencyDto(competencySeed, domain.id, path.id),
        );
