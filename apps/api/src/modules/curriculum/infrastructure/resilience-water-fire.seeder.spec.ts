...
import { ResilienceWaterFireSeeder } from './resilience-water-fire.seeder.js';
import { buildResilienceWaterFireSeedData } from './resilience-water-fire.seed-data.js';

const DOMAIN_ID = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a16';

const seedDataForPathIds = buildResilienceWaterFireSeedData();
const PATH_ID_BY_CODE = new Map(
  seedDataForPathIds.paths.map((p, i) => [p.path.code, `b0eebc99-9c0b-4ef8-bb6d-6bb9bd380f${String(i).padStart(2, '0')}`]),
);
...
describe('ResilienceWaterFireSeeder', () => {
  ...
  it('creates every path and its competencies when the RESILIENCE domain already exists (the normal case)', async () => {
    ...
    const seeder = new ResilienceWaterFireSeeder(definitionsService);
