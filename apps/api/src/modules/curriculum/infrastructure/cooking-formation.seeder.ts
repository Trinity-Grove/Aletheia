...
import {
  buildCookingFormationSeedData,
  buildCookingFormationDomainDto,
  buildCookingFormationPathDto,
  buildCookingFormationCompetencyDto,
} from './cooking-formation.seed-data.js';

export interface CookingFormationSeedResult {
  domainCreated: boolean;
  pathCreated: boolean;
  competenciesCreated: number;
}

// Installs the "Culinária" foundational content from issue #95 section
// 18 (domain -> path -> competencies) -- same pattern as
// BiblicalFormationSeeder (issue #95 section 5, PR #131),
// MusicFormationSeeder (issue #95 section 13, PR #132), and
// TradesFormationSeeder (issue #95 section 15, PR #134).
//
...
@Injectable()
export class CookingFormationSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<CookingFormationSeedResult> {
    const data = buildCookingFormationSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(buildCookingFormationDomainDto(data.domain));
    ...
    if (!path) {
      path = await this.definitionsService.createLearningPath(buildCookingFormationPathDto(data.path, domain.id));
    ...
    for (const competencySeed of data.competencies) {
      ...
      const created = await this.definitionsService.createCompetencyDefinition(
        buildCookingFormationCompetencyDto(competencySeed, domain.id, path.id),
      );
