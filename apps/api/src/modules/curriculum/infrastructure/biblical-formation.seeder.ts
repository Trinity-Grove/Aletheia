...
import {
  buildBiblicalFormationSeedData,
  buildBiblicalFormationDomainDto,
  buildBiblicalFormationPathDto,
  buildBiblicalFormationCompetencyDto,
} from './biblical-formation.seed-data.js';

export interface BiblicalFormationSeedResult {
  domainCreated: boolean;
  pathCreated: boolean;
  competenciesCreated: number;
}

// Installs the "Formação Bíblica" introductory-tier content from issue
// #95 section 5 (domain -> path -> competencies).
//
...
@Injectable()
export class BiblicalFormationSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<BiblicalFormationSeedResult> {
    const data = buildBiblicalFormationSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(buildBiblicalFormationDomainDto(data.domain));
    ...
    if (!path) {
      path = await this.definitionsService.createLearningPath(buildBiblicalFormationPathDto(data.path, domain.id));
    ...
    for (const competencySeed of data.competencies) {
      ...
      const created = await this.definitionsService.createCompetencyDefinition(
        buildBiblicalFormationCompetencyDto(competencySeed, domain.id, path.id),
      );
