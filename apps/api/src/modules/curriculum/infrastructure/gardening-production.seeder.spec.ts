...
import { GardeningProductionSeeder } from './gardening-production.seeder.js';
import { buildGardeningProductionSeedData } from './gardening-production.seed-data.js';

const DOMAIN_ID = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380b11';
const PATH_ID = 'b0eebc99-9c0b-4ef8-bb6d-6bb9bd380b22';

describe('GardeningProductionSeeder', () => {
  const seedData = buildGardeningProductionSeedData();

  it('creates the path and every competency when the GARDENING domain already exists (the normal case)', async () => {
    ...
    const seeder = new GardeningProductionSeeder(definitionsService);
  ...
  it('creates only the missing competencies when the path already exists but a competency is new', async () => {
    const existingDomain = { id: DOMAIN_ID, code: seedData.domain.code, version: 1, status: 'PUBLISHED' };
