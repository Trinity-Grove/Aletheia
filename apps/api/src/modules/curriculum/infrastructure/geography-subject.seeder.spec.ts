...
import { GeographySubjectSeeder } from './geography-subject.seeder.js';
import { buildGeographySubjectSeedData } from './geography-subject.seed-data.js';

const DOMAIN_ID = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a12';
...
// geography-subject.seed-data.ts eventually adds, with no collision risk.
const seedDataForPathIds = buildGeographySubjectSeedData();
const PATH_ID_BY_CODE = new Map(
  seedDataForPathIds.paths.map((p, i) => [p.path.code, `b0eebc99-9c0b-4ef8-bb6d-6bb9bd380b${String(i).padStart(2, '0')}`]),
);
...
describe('GeographySubjectSeeder', () => {
  ...
  it('creates the domain, every grade-band path and every competency on a first run', async () => {
    ...
    const seeder = new GeographySubjectSeeder(definitionsService);
