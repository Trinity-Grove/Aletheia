...
import {
  buildTradesFormationSeedData,
  buildTradesFormationDomainDto,
  buildTradesFormationPathDto,
  buildTradesFormationCompetencyDto,
} from './trades-formation.seed-data.js';

export interface TradesFormationSeedResult {
  domainCreated: boolean;
  pathCreated: boolean;
  competenciesCreated: number;
}

// Installs the "Ofícios" foundational content from issue #95 section 15
// (domain -> path -> competencies) -- same pattern as
// BiblicalFormationSeeder (issue #95 section 5, PR #131) and
...
@Injectable()
export class TradesFormationSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<TradesFormationSeedResult> {
    const data = buildTradesFormationSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(buildTradesFormationDomainDto(data.domain));
    ...
    if (!path) {
      path = await this.definitionsService.createLearningPath(buildTradesFormationPathDto(data.path, domain.id));
    ...
    for (const competencySeed of data.competencies) {
      ...
      const created = await this.definitionsService.createCompetencyDefinition(
        buildTradesFormationCompetencyDto(competencySeed, domain.id, path.id),
      );
