...
import {
  buildMathSubjectSeedData,
  buildMathSubjectDomainDto,
  buildMathSubjectPathDto,
  buildMathSubjectCompetencyDto,
} from './math-subject.seed-data.js';

export interface MathSubjectSeedResult {
  domainCreated: boolean;
  pathsCreated: number;
  competenciesCreated: number;
}

// Installs the "Matemática" core-academic-subject content (domain ->
// multiple grade-band paths -> competencies). First of five planned core
// subjects (Português, História, Geografia, Ciências follow as separate
// tasks) -- see math-subject.seed-data.ts for the full rationale.
//
// Shape difference from every prior seeder (BiblicalFormationSeeder,
// MusicFormationSeeder, TradesFormationSeeder, CookingFormationSeeder,
// GardeningFormationSeeder, ResilienceFormationSeeder): those each had
// exactly one foundational LearningPath. This domain has multiple
// LearningPaths (one per grade band), so the seeding loop is one level
// deeper -- domain once, then for each grade band: path, then its
// competencies.
//
...
// window.
@Injectable()
export class MathSubjectSeeder {
  constructor(private readonly definitionsService: DefinitionsService) {}

  async seed(): Promise<MathSubjectSeedResult> {
    const data = buildMathSubjectSeedData();
    ...
    if (!domain) {
      domain = await this.definitionsService.createLearningDomain(buildMathSubjectDomainDto(data.domain));
    ...
    for (const pathData of data.paths) {
      ...
      if (!path) {
        path = await this.definitionsService.createLearningPath(
          buildMathSubjectPathDto(pathData.path, domain.id),
        );
      ...
      for (const competencySeed of pathData.competencies) {
        ...
        const created = await this.definitionsService.createCompetencyDefinition(
          buildMathSubjectCompetencyDto(competencySeed, domain.id, path.id),
        );
