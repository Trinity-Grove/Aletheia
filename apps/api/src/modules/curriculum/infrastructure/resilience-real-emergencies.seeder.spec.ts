...
import { ResilienceRealEmergenciesSeeder } from './resilience-real-emergencies.seeder.js';
import { buildResilienceRealEmergenciesSeedData } from './resilience-real-emergencies.seed-data.js';

const DOMAIN_ID = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a17';

const seedDataForPathIds = buildResilienceRealEmergenciesSeedData();
const PATH_ID_BY_CODE = new Map(
  seedDataForPathIds.paths.map((p, i) => [p.path.code, `b0eebc99-9c0b-4ef8-bb6d-6bb9bd3810${String(i).padStart(2, '0')}`]),
);
...
describe('ResilienceRealEmergenciesSeeder', () => {
  ...
  it('creates every path and its competencies when the RESILIENCE domain already exists (the normal case)', async () => {
    ...
    const seeder = new ResilienceRealEmergenciesSeeder(definitionsService);
  ...
  it('creates the RESILIENCE domain defensively if it does not exist yet (standalone run on a fresh database)', async () => {
    const definitionsService = {
  ...
  it('creates only the missing paths and their competencies when some paths already exist', async () => {
    const existingDomain = { id: DOMAIN_ID, code: seedData.domain.code, version: 1, status: 'PUBLISHED' };
    ...
    const existingCompetencies = firstPathData.competencies.slice(0, 3).map((c, i) => ({
      id: `existing-${i}`,
      code: c.code,
      version: 1,
      status: 'PUBLISHED',
    ...
    const remainingCompetenciesCount = totalCompetencies - existingCompetencies.length;
