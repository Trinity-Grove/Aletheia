...
import { ScienceSubjectSeeder } from '../src/modules/curriculum/infrastructure/science-subject.seeder.js';
import { PrismaService } from '../src/platform/database/prisma.service.js';
import { buildScienceSubjectSeedData } from '../src/modules/curriculum/infrastructure/science-subject.seed-data.js';

// Proves the "Ciências" core-academic-subject seed actually publishes a
...
// as MathSubjectSeeder (#142/#145) and every prior content seed.
describe('ScienceSubjectSeeder (real Postgres)', () => {
  ...
  it('keeps each grade band age-appropriate -- no advanced-band concepts leaked into an earlier band, and each later band genuinely escalates', async () => {
    // Same reasoning as math-subject-seed.integration-spec.ts: all three
    // bands intentionally build on the same subject, so the meaningful
    // check isn't "these bands never share vocabulary" -- it's that each
    ...
    const earlyYearsText = textFor('SCIENCE.EARLY_YEARS');
    const primaryGrammarText = textFor('SCIENCE.PRIMARY_GRAMMAR');
    const middleLogicText = textFor('SCIENCE.MIDDLE_LOGIC');

    // EARLY_YEARS is observational only -- no formal biology/physics/
    // chemistry vocabulary yet. Word stems (single words, not full
    // multi-word singular/plural phrases), per the substring bug already
    // caught once in the Matemática seed's equivalent test -- a phrase
    // like "cadeia alimentar" would NOT substring-match "cadeias
    // alimentares" (the "s" breaks the phrase mid-match even though each
    // word's stem alone matches fine).
    const primaryGrammarOnwardTerms = ['corpo humano', 'ciclo', 'evaporação', 'físic', 'máquina', 'ímã', 'sombra'];
    const middleLogicOnlyTerms = [
      'ecossistema',
      'cadeia',
      'aliment',
      'célul',
    ...
    // PRIMARY_GRAMMAR must escalate beyond EARLY_YEARS (contains several
    // of its own concepts, trivially true by construction) but must not
    // yet reach into MIDDLE_LOGIC-only territory (ecosystems, cells,
    // tectonics, solar system, chemistry) -- that's the next band's job.
    ...
    assertAgeBounds('SCIENCE.EARLY_YEARS', { maxCeiling: 6 });
    assertAgeBounds('SCIENCE.PRIMARY_GRAMMAR', { minFloor: 6, maxCeiling: 10 });
    assertAgeBounds('SCIENCE.MIDDLE_LOGIC', { minFloor: 10, maxCeiling: 14 });
  });

  it('stays scoped to the current bands -- no Ensino Médio physics/chemistry/biology-split terminology leaked in', async () => {
    // Sanity check for the scope boundary documented in
    // science-subject.seed-data.ts: HIGH_RHETORIC (a real physics/
    // chemistry/biology split) is explicitly out of scope for this
    // slice. This asserts that boundary was actually respected in the
    // content, not just claimed in a comment.
    const forbiddenTerms = ['física clássica', 'termodinâmica', 'estequiometria', 'genética', 'tabela periódica', 'cinemática', 'eletromagnetismo'];
    const haystack = [
