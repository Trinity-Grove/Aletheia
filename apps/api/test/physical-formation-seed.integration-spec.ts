...
import { PhysicalFormationSeeder } from '../src/modules/curriculum/infrastructure/physical-formation.seeder.js';
import { PrismaService } from '../src/platform/database/prisma.service.js';
import { buildPhysicalFormationSeedData } from '../src/modules/curriculum/infrastructure/physical-formation.seed-data.js';

// Proves the "Formação Física" foundational seed (issue #95 section 23)
// actually publishes a real, internally-consistent domain -> path ->
...
// GardeningFormationSeeder (#136) and the other Domain -> Path ->
// Competency seeders.
describe('PhysicalFormationSeeder (real Postgres)', () => {
  let app: NestFastifyApplication;
  let prisma: PrismaService;
  const seedData = buildPhysicalFormationSeedData();
  ...
  it('stays neutral to sport/martial-art brand choice and to RESILIENCE outdoor/survival content -- no scope leakage', async () => {
    // Sanity check for the scope boundaries documented in
    // physical-formation.seed-data.ts: this slice must not favor a
    // single sport or martial-arts style, and its "Atividades Outdoor"
    // competency (fitness through outdoor movement) must not encroach on
    // RESILIENCE's separate navigation/camping/survival content.
    const forbiddenTerms = [
      // A sample of specific sports/martial-arts styles that would
      // indicate the content stopped being style-agnostic.
      'futebol',
      'vôlei',
      'basquete',
      'judô',
      ...
      'jiu-jitsu',
      'muay thai',
      // RESILIENCE-domain-specific navigation/camping/survival/first-aid
      // terms (resilience-navigation-camping.seed-data.ts,
      // resilience-water-fire.seed-data.ts,
      // resilience-real-emergencies.seed-data.ts,
      // resilience-formation.seed-data.ts).
      'bússola',
      'barraca',
      'abrigo',
      'mapa',
