...
import { buildMusicFormationSeedData } from '../src/modules/curriculum/infrastructure/music-formation.seed-data.js';

// Proves the "Musicalização" foundational seed (issue #95 section 13)
...
// same shape of proof as BiblicalFormationSeeder's integration spec (#131).
describe('MusicFormationSeeder (real Postgres)', () => {
  let app: NestFastifyApplication;
  let prisma: PrismaService;
  const seedData = buildMusicFormationSeedData();
  ...
  it('stays instrument-agnostic and worship-tradition-neutral -- no Instrumentos or Música e Cristianismo content leaked into this slice', async () => {
    // Sanity check for the scope boundary documented in
    // music-formation.seed-data.ts: this slice covers only section 13's
    // "Fundamentos" subsection. "Instrumentos" (specific instruments) and
    // "Música e Cristianismo" (a specific worship-music tradition) are
    // explicitly out of scope for this PR -- this asserts that boundary
