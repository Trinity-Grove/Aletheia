...
import { PersonalFinanceFoundationsSeeder } from '../src/modules/curriculum/infrastructure/personal-finance-foundations.seeder.js';
import { PrismaService } from '../src/platform/database/prisma.service.js';
import { buildPersonalFinanceFoundationsSeedData } from '../src/modules/curriculum/infrastructure/personal-finance-foundations.seed-data.js';

// Proves the "Educação Financeira Prática" foundational seed (issue #95
// section 22) actually publishes a real, internally-consistent domain ->
// path -> competencies hierarchy against real Postgres, and that
// re-running it is idempotent (no duplicates, no altered rows) -- same
// shape of proof as HomeSufficiencyFoundationsSeeder (#168) and
// GardeningFormationSeeder (#136).
describe('PersonalFinanceFoundationsSeeder (real Postgres)', () => {
  let app: NestFastifyApplication;
  let prisma: PrismaService;
  const seedData = buildPersonalFinanceFoundationsSeedData();
  ...
  it('stays neutral to content already owned by Culinária and Autossuficiência Doméstica -- no food-specific or non-financial logistics terms leaked in', async () => {
    // Sanity check for the scope boundary documented in
    // personal-finance-foundations.seed-data.ts: this slice covers
    // GENERAL personal/family finance, never the food-specific purchase
    // logistics already owned by COOKING.LIFE_SKILLS (PR #165) or the
    // non-food household purchase/inventory logistics already owned by
    // HOME_SUFFICIENCY.FOUNDATIONS (PR #168). This asserts that boundary
    // was actually respected in the content, not just claimed in a
    // comment.
    const forbiddenTerms = [
      // Cozinhar / Culinária / COOKING.LIFE_SKILLS
      'receita',
      'refeição',
      'despensa',
      'ingrediente',
      'cardápio',
      // Autossuficiência Doméstica / HOME_SUFFICIENCY.FOUNDATIONS
      'limpeza',
      'lavagem de roupas',
